...
int EventLoop::AddTask(Channel* channel, ElemType type) {
    ...
    if (m_threadID == this_thread::get_id()) {
        ...
        //  满足条件，两个文件描述符，可以相互通信，//1，使用pipe进程间通信，进程更可，//2，socketpair
        //  文件描述符进行通信
...
int EventLoop::Add(Channel* channel) {
    ...
    if (m_channelmap.find(fd) == m_channelmap.end()) {
        m_channelmap.insert(
            make_pair(fd, channel));        // 将当前fd和channel添加到map
...
int EventLoop::readlocalMessage(void* arg) {
    ...
    char buf[256];
    read(evLoop->m_socketPair[1], buf,
         sizeof(buf));  // 目的仅为触发一次读事件，检测文件描述符解除阻塞
    return 0;
}

int EventLoop::readMessage() {
    char buf[256];
    read(m_socketPair[1], buf,
         sizeof(buf));  // 目的仅为触发一次读事件，检测文件描述符解除阻塞
    return 0;
}
