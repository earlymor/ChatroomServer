...
class Buffer {
   public:
    Buffer(
        int size);  // 初始化Buffer,size申请大小,可根据Buffer读写指针进行相关操作
    ~Buffer();      // 销毁内存函数
    ...
    // 得到剩余可写的内存容量
    inline int writeableSize() { return m_capacity - m_readPos; };
    ...
    // 写内存 1，直接写，2，接受套接字数据
    // buffer
    // ,添加的字符串，字符串data对应的长度,把data所指向的数据存入到buffer中去
    ...
    // 得到读数据与写数据之间的距离
    inline int len() { return (m_writePos - m_readPos); }
